...
for custom MLP models (small / medium / large).

Usage examples:

    # Single run
    python benchmark_crypten_mlp.py --model small --batch-size 32 --device cpu
...
class SmallMLP(nn.Module):
    ...
    def __init__(self, num_classes: int = 10, input_dim: int = 3 * 32 * 32):
        super().__init__()
...
def build_mlp(model_name: str, num_classes: int = 10) -> nn.Module:
    model_name = model_name.lower()
    if model_name == "small":
        return SmallMLP(num_classes=num_classes)
    elif model_name == "medium":
        return MediumMLP(num_classes=num_classes)
    elif model_name == "large":
        return LargeMLP(num_classes=num_classes)
    else:
...
def run_single_experiment(
    ...
    model = build_mlp(model_name, num_classes=10)
...
def main():
    ...
    if args.sweep:
        ...
        # Single-run mode
        model = build_mlp(args.model, num_classes=10)
